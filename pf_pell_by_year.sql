...
WHERE stud.School_Code NOT IN ('SS','SH','AU','BC','TC','GP','TEST') and SUBSTRING(stud.Term_Identifier,1,4) >= '2015' and SUBSTRING(stud.Term_Identifier,1,4) <= '2024'
--joining financial aid data based on enrolled population
drop table if exists #finaid
select
CAST(stay.award_year_token AS VARCHAR(4)) as Award_Year, 
...
INNER JOIN powerfaids..stu_award_year stay on STUDENT.student_token = stay.student_token
inner join powerfaids..stu_award as STU_AWARD ON STU_AWARD.stu_award_year_token = stay.stu_award_year_token
inner join powerfaids..funds as FUNDS ON STU_AWARD.fund_ay_token = FUNDS.fund_token
 inner join #pop p on STUDENT.alternate_id = p.Student and p.Year = FUNDS.award_year_token
where stay.award_year_token >= '2015' and stay.award_year_token <= '2024'
